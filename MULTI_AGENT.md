...
11. 主 Agent 将本轮提交推送到已配置的远程 GitHub 仓库，确认远程分支已包含该提交；只在推送成功后才把本轮标记为完成。
12. 若用户已授权持续迭代，主 Agent 进入下一轮“架构确认 → 任务拆分 → 子 Agent 并行 → 集成验收”；否则只报告计划并等待用户确认。
...
每完成一轮可独立验收的任务，必须由主 Agent 统一创建一次 Git 提交并推送到远程 GitHub 仓库。子 Agent 不自行提交或推送，避免产生边界不清、包含半成品或远程状态不一致的提交。
...
- 使用当前分支对应的远程跟踪分支；首次推送新分支时设置 upstream。
- 默认推送到 `origin`；不得使用强制推送，不得重写共享分支历史。
- 推送后核对本地 `HEAD`、远程跟踪分支和提交哈希一致，不能只根据命令退出码假定成功。
- 推送失败时立即报告认证、网络、分支保护或远程冲突等原因；本地 commit 不等于本轮完成。
- 若任务应通过功能分支和 PR 交付，遵循用户指定的分支/PR 流程；没有明确要求时沿用当前项目已经确认的分支策略。
...
不要自行提交或推送 Git；由主 Agent 在本轮验收通过后统一提交并推送到远程 GitHub 仓库。
```
...
- 定向测试与全部 `unittest` 通过。
- 主 Agent 已创建只包含本轮内容的 Git 提交、成功推送到远程 GitHub 仓库，并在交付报告中标注本地/远程提交哈希、提交信息、远程分支和文件范围。
